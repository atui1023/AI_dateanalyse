...
function exportWorkflowResult(run: any, format: 'csv' | 'xlsx') { window.open(api.workflowExportUrl(run.id, format), '_blank', 'noopener,noreferrer') }
function stepPreview(output: any) { return output?.preview || output?.files?.[0]?.preview || null }
function previewRows(preview: any) { return (preview?.rows || []).map((row: any[]) => Object.fromEntries((preview.columns || []).map((column: string, index: number) => [column, row[index]]))) }
...
.preview-label { grid-column:1 / -1; color:var(--text-secondary); font-size:12px; margin-top:4px; }
